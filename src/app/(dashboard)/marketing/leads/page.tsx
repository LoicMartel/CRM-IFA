...
export default async function MarketingLeadsPage() {
  ...
  const { data: leads } = await supabase
    .from("contacts")
    .select("id, first_name, last_name, email, phone, company_id, lifecycle_stage, lead_status, created_at, companies!contacts_company_id_fkey(name), lead_sources!contacts_source_id_fkey(name), team_members!contacts_owner_id_fkey(first_name, last_name)")
  ...
  return (
    <>
      ...
      <div className="p-6 space-y-6">
        <div className="grid gap-3 md:grid-cols-2" style={{ maxWidth: 500 }}>
          ...
          <div className="lca-card" style={{ padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#8399a9" }}>Propriétaire</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#1a2a3a" }}>Pauline</div>
            </div>
            <UserPlus style={{ width: 16, height: 16, color: "#8399a9" }} />
          </div>
        </div>
