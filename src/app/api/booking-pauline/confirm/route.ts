  ...
  if (meetingId && isStepActive(wf, "trigger-notifications").active) {
    ...
    after(async () => {
      try {
        ...
        if (!result.success) console.error(`[booking-pauline] notification failed for meeting ${mid}:`, result.error);
        const failures = result.results.filter(r => !["Envoye", "Ajoute"].includes(r.status));
        if (failures.length > 0) console.error(`[booking-pauline] partial failures for meeting ${mid}:`, JSON.stringify(failures));
      } catch (err) {
        console.error(`[booking-pauline] notification error for meeting ${mid}:`, err);
      }
    });
  }
