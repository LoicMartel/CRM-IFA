  ...
  if (meetingId && isStepActive(wf, "trigger-notifications").active) {
    ...
    after(async () => {
      try {
        ...
        if (!result.success) console.error(`[booking-general] notification failed for meeting ${mid}:`, result.error);
        const failures = result.results.filter(r => !["Envoye", "Ajoute"].includes(r.status));
        if (failures.length > 0) console.error(`[booking-general] partial failures for meeting ${mid}:`, JSON.stringify(failures));
      } catch (err) {
