...
import { KeyRound, User, Mail, Check, Camera, Link, MessageSquare, Calendar, ChevronDown, ChevronRight, Copy, Send, ExternalLink, Plus, Pencil, Trash2 } from "lucide-react";
...
function ResourceLinksManager() {
  ...
  return (
    <div className="lca-card" style={{ padding: 0, overflow: "hidden" }}>
      <div style={{ height: 4, background: "linear-gradient(90deg, #8e44ad 0%, #2980b9 50%, #27ae60 100%)" }} />
      <div style={{ padding: "20px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <h3 style={{ fontWeight: 700, fontSize: 16, color: "#1a2a3a", display: "flex", alignItems: "center", gap: 8, margin: 0 }}>
            <ExternalLink className="h-5 w-5" style={{ color: "#1E2A5A" }} /> Ressources
          </h3>
          {!adding && (
            <button
              onClick={() => { setAdding(true); setEditingId(null); setForm({ name: "", description: "", url: "", category: "commercial" }); }}
              style={{
                display: "flex", alignItems: "center", gap: 6, height: 32, borderRadius: 8, border: "none", cursor: "pointer",
                background: "linear-gradient(135deg, #0f1630 0%, #1E2A5A 100%)", color: "white", fontSize: 12, fontWeight: 700, padding: "0 14px",
              }}
            >
        ...
        <p style={{ fontSize: 12, color: "#8399a9", marginBottom: 16 }}>
          Configurez les liens rapides qui apparaissent dans la page Ressources Commerciales.
        </p>
        ...
        {adding && (
          <div style={{ background: "#f8fbfd", borderRadius: 10, padding: 16, marginBottom: 16, border: "1px solid #e8ecf1" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
              <div>
                ...
                <input
                  ...
                  style={{ width: "100%", height: 36, borderRadius: 8, border: "1px solid #dce8f0", padding: "0 12px", fontSize: 13 }}
                />
              </div>
              <div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button onClick={cancelForm} style={{ height: 32, borderRadius: 8, background: "#e8ecf1", color: "#5a6f80", fontSize: 12, fontWeight: 600, padding: "0 16px", border: "none", cursor: "pointer" }}>
                Annuler
              </button>
              <button
                onClick={handleSave}
                ...
                style={{
                  ...
                  background: (!form.name.trim() || !form.url.trim()) ? "#e8ecf1" : "linear-gradient(135deg, #E8732A 0%, #e65100 100%)",
                  color: (!form.name.trim() || !form.url.trim()) ? "#8399a9" : "white",
                  fontSize: 12, fontWeight: 700, padding: "0 20px",
                  opacity: saving ? 0.6 : 1,
                }}
              >
        ...
        {grouped.length === 0 && !adding && (
          <div style={{ textAlign: "center", padding: "20px 0", color: "#8399a9", fontSize: 13 }}>
            Aucune ressource configurée. Cliquez sur &quot;Ajouter&quot; pour créer votre première ressource.
          </div>
        )}
