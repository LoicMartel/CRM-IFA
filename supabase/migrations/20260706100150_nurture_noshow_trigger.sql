...
-- SECURITY DEFINER (+ search_path figé) pour insérer malgré la RLS.
-- ⚠️ Le corps est enveloppé dans un bloc EXCEPTION : le nurturing est best-effort et ne doit JAMAIS
-- faire échouer le marquage no_show du commercial (table appliquée après ce trigger, unique_violation
-- sur ré-enrôlement, FK, etc. -> l'UPDATE meetings passe quand même).
